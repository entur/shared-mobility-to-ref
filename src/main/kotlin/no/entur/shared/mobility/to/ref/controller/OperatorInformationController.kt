...
@RestController("no.entur.shared.mobility.to.ref.controller.OperatorInformationController")
@Validated
@RequestMapping("\${api.base-path:/bike}")
class OperatorInformationController(
    @Autowired(required = true) val service: OperatorInformationService,
) {
    @Operation(
    ...
    fun operatorAlertsGet(
        ...
        ) @Valid @RequestParam(value = "regionId", required = false) regionId: kotlin.String?,
        @Parameter(
            description = "optional id of the station to use in the filter (/operator/stations)",
        ) @Valid @RequestParam(value = "stationId", required = false) stationId: kotlin.String?,
    ): ResponseEntity<List<SystemAlert>> =
        ResponseEntity(
            service.operatorAlertsGet(acceptLanguage, api, apiVersion, maasId, addressedTo, offset, limit, regionId, stationId),
    ...
    fun operatorAvailableAssetsGet(
        ...
        ResponseEntity(
            service.operatorAvailableAssetsGet(acceptLanguage, api, apiVersion, maasId, addressedTo, offset, limit, regionId, stationId),
            HttpStatus.valueOf(200),
        )
    ...
    fun operatorInformationGet(
        ...
        ResponseEntity(service.operatorInformationGet(acceptLanguage, api, apiVersion, maasId, addressedTo), HttpStatus.valueOf(200))
    ...
    fun operatorMetaGet(
        ...
        ResponseEntity(service.operatorMetaGet(acceptLanguage, maasId, addressedTo), HttpStatus.valueOf(200))
    ...
    fun operatorOperatingCalendarGet(
        ...
        ResponseEntity(
            service.operatorOperatingCalendarGet(acceptLanguage, api, apiVersion, maasId, addressedTo, regionId, stationId),
            HttpStatus.valueOf(200),
        )
    ...
    ): ResponseEntity<Unit> = ResponseEntity(service.operatorPingGet(acceptLanguage), HttpStatus.valueOf(200))
    ...
    fun operatorStationsGet(
        ...
        @DecimalMin(
            "0",
        )@Parameter(description = "the longitude of the search location (WGS84)") @Valid @RequestParam(value = "lon", required = false) lon:
            kotlin.Float?,
        @DecimalMin(
            "0",
        )@Parameter(description = "the latitude of the search location (WGS84)") @Valid @RequestParam(value = "lat", required = false) lat:
            kotlin.Float?,
        @DecimalMin(
            "0",
        )@Parameter(
            description = "the range in meters from the search location to look for stations",
        ) @Valid @RequestParam(value = "radius", required = false) radius: kotlin.Float?,
    ): ResponseEntity<List<StationInformation>> =
        ResponseEntity(
            service.operatorStationsGet(acceptLanguage, api, apiVersion, maasId, addressedTo, offset, limit, regionId, lon, lat, radius),
            HttpStatus.valueOf(200),
        )
}
