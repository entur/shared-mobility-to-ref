...
data class BookingOperation(
    @Schema(
        example = "null",
        required = true,
        description = """the operation that is requested. When extra time is needed to complete the initial booking, EXTEND_EXPIRY_TIME 
    ...
    @Schema(
        example = "null",
        description = """This operation can be done on behalf of another party. The MP can act on behalf of the END_USER (cancel this 
            |booking for me); to override the default origin. In case this field is missing, it must be assumed that the events the MP is 
            |sending, this field should contain "MP". And in case the TO is sending, "TO".""",
    )
