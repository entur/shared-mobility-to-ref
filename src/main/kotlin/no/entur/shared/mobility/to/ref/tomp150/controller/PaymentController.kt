...
@RestController("no.entur.shared.mobility.to.ref.tomp150.controller.PaymentController")
@Validated
...
class PaymentController(@Autowired(required = true) val service: PaymentService) {
    ...
    fun paymentJournalEntryGet(@Parameter(description = "A list of the languages/localizations the user would like to see the results in. For user privacy and ease of use on the TO side, this list should be kept as short as possible, ideally just one language tag from the list in operator/information", `in` = ParameterIn.HEADER, required = true) @RequestHeader(value = "Accept-Language", required = true) acceptLanguage: kotlin.String,@Parameter(description = "API description, can be TOMP or maybe other (specific/derived) API definitions", `in` = ParameterIn.HEADER, required = true) @RequestHeader(value = "Api", required = true) api: kotlin.String,@Parameter(description = "Version of the API.", `in` = ParameterIn.HEADER, required = true) @RequestHeader(value = "Api-Version", required = true) apiVersion: kotlin.String,@Parameter(description = "The ID of the sending maas operator", `in` = ParameterIn.HEADER, required = true) @RequestHeader(value = "maas-id", required = true) maasId: kotlin.String,@Parameter(description = "The ID of the maas operator that has to receive this message", `in` = ParameterIn.HEADER) @RequestHeader(value = "addressed-to", required = false) addressedTo: kotlin.String?,@Parameter(description = "start of the selection") @Valid @RequestParam(value = "from", required = false) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE_TIME) from: java.time.OffsetDateTime?,@Parameter(description = "end of the selection") @Valid @RequestParam(value = "to", required = false) @org.springframework.format.annotation.DateTimeFormat(iso = org.springframework.format.annotation.DateTimeFormat.ISO.DATE_TIME) to: java.time.OffsetDateTime?,@Parameter(description = "", schema = Schema(allowableValues = ["TO_INVOICE", "INVOICED"])) @Valid @RequestParam(value = "state", required = false) state: JournalState?,@Parameter(description = "") @Valid @RequestParam(value = "id", required = false) id: kotlin.String?,@Parameter(description = "type of booking line (e.g. fare, addition costs, fines, ...)", schema = Schema(allowableValues = ["ALL", "DAMAGE", "LOSS", "STOLEN", "EXTRA_USAGE", "REFUND", "FINE", "OTHER_ASSET_USED", "CREDIT", "VOUCHER", "DEPOSIT", "OTHER", "FARE"])) @Valid @RequestParam(value = "category", required = false) category: JournalCategory?,@Min(0)@Parameter(description = "start of the selection", schema = Schema(defaultValue = "0")) @Valid @RequestParam(value = "offset", required = false, defaultValue = "0") offset: kotlin.Int,@Min(0)@Parameter(description = "count of the selection") @Valid @RequestParam(value = "limit", required = false) limit: kotlin.Int?): ResponseEntity<List<JournalEntry>> {
        return ResponseEntity(service.paymentJournalEntryGet(acceptLanguage, api, apiVersion, maasId, addressedTo, from, to, state, id, category, offset, limit), HttpStatus.valueOf(200))
    }
}
