...
import no.entur.shared.mobility.to.ref.dto.ConnectedLegInfo
import no.entur.shared.mobility.to.ref.dto.Customer
import no.entur.shared.mobility.to.ref.dto.Place
import no.entur.shared.mobility.to.ref.dto.Traveler
...
data class OneStopBookingRequest(
    ...
    @Schema(
        example = "null",
        description = "instead of using the from/to construct, it is also possible to give an indication of the distance to travel. The process identifier 'USE_ESTIMATED_DISTANCE' is used to indicate this scenario. Also in meters",
    )
    @get:JsonProperty("estimatedDistance") val estimatedDistance: kotlin.Int? = null,
    @Schema(
        example = "null",
        description = "The intended departure time. If left out and no arrivalTime is set, the current time should be assumed. If only the arrival time is specified, this is an implicit request for a guaranteed arrival at that time.",
    )
