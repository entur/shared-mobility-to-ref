...
@RestController("no.entur.shared.mobility.to.ref.controller.BookingOptionalController")
@Validated
@RequestMapping("\${api.base-path:/bike}")
class BookingOptionalController(
    @Autowired(required = true) val service: BookingOptionalService,
) {
    @Operation(
        summary = "",
    ...
    fun bookingsIdPut(
        ...
        ResponseEntity(service.bookingsIdPut(acceptLanguage, api, apiVersion, maasId, id, booking, addressedTo), HttpStatus.valueOf(200))
