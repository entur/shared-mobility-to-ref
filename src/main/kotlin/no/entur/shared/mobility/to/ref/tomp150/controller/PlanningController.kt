...
@RestController("no.entur.shared.mobility.to.ref.tomp150.controller.PlanningController")
@Validated
...
class PlanningController(@Autowired(required = true) val service: PlanningService) {
    ...
    fun planningsPost(
        ...
        ) @Valid @RequestParam(value = "booking-intent", required = false, defaultValue = "false") bookingIntent: Boolean,
        @Parameter(description = "") @Valid @RequestBody(required = false) planningRequest: PlanningRequest?
    ): ResponseEntity<Planning> {
        return ResponseEntity(
